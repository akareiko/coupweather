...
export default function RainWithClouds() {
  ...
  useEffect(() => {
    ...
    // Create Raindrops
    const raindrops = [];
    const rainGeometry = new THREE.CylinderGeometry(0.002, 0.002, 0.07, 8); // Small sphere for raindrop
    const rainMaterial = new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.15 });
    ...
    const animate = () => {
      ...
      raindrops.forEach((raindrop) => {
        raindrop.position.y -= Math.random() * 0.1 + 0.1; // Falling speed
        if (raindrop.position.y < -10) {
          raindrop.position.y = Math.random() * 20 + 10; // Reset position to top
          raindrop.position.x = Math.random() * 20 - 10; // Randomize horizontal position
        }
      });
